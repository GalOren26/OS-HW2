...
int encrypt_block(parssing_data* p_params)
{
	/* the function encrypt file by positiong in the file and parms */
	if (p_params->end_pos == p_params->start_pos)
	{
		return SUCCESS;
	}
...
DWORD WINAPI EncThread(LPVOID lpParam)
{
	parssing_data* params;
	params = (parssing_data*)lpParam;
	int ret_val = encrypt_block(params);
...
void Createmultiplethreads_enc(int num_of_threads, uli num_of_lines, uli* end_of_lines, parssing_data* p_params)
{
	...
	for (int i = 0; i < num_of_threads; i++)
	{
		...
		thread_params[i]->input_path = p_params->input_path;
		thread_params[i]->key = p_params->key;
		p_thread_handles[i] = CreateThreadSimple(EncThread, (LPVOID)thread_params[i], &p_thread_ids[i]);
	}
	wait_code = WaitForMultipleObjects(num_of_threads, p_thread_handles, 1, TIMEOUT_IN_MILLISECONDS);
	FreeHandelsArray(p_thread_handles, num_of_threads);
	FreeArray(thread_params, num_of_threads);
